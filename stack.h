...
public:
	...
	void Push(T item){ //проталкиваем элементы "снизу вверх"
		if (count == size-1)
        cout << "Stek polon";
        else{
            ++count;
	...
	T Top(){ //получение значения без выборки
		if (count == -1){
			cout << "Stek pust!" << endl;
			return -1;
		}
